...
$sql = "SELECT id, product_name, price, cost_price, stock_quantity, barcode, pieces_per_carton FROM products ORDER BY stock_quantity > 0 DESC, product_name ASC";
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .product-card.out-of-stock { opacity: 0.5; cursor: not-allowed; background: #f9f9f9; }
        .product-card.out-of-stock:hover { transform: none; box-shadow: 0 5px 15px rgba(0, 0, 0, 0.05); border-color: #e0e7ff; }
        ...
        .btn-load-more { background: var(--secondary); color: white; padding: 12px 25px; border: none; border-radius: 8px; cursor: pointer; font-size: 1.1rem; }
        ...
        .modal-content { background: white; padding: 30px; border-radius: 15px; width: 90%; max-width: 550px; box-shadow: 0 5px 20px rgba(0,0,0,0.3); }
        ...
        .modal-footer .btn-primary { background: var(--primary); color: white; flex-grow: 1; }
        .modal-footer .btn-secondary { background: var(--gray); color: white; }
        
        #receipt-modal-content { max-width: 420px; }
        #receipt-body { text-align: left; }
        #receipt-body h2 { font-size: 1.6rem; margin-bottom: 10px; text-align: center; }
        #receipt-body p { text-align: center; margin: 2px 0; }
        #receipt-items-table { width: 100%; text-align: left; margin-top: 20px; border-collapse: collapse; }
        #receipt-items-table th, #receipt-items-table td { padding: 8px; border-bottom: 1px dashed #ccc; }
        #receipt-items-table th { border-bottom: 2px solid #333; }
        #receipt-total { margin-top: 20px; font-size: 1.4rem; font-weight: bold; text-align: right; }

        @media print {
            body, .container, header, .layout, .products-section, .cart-container, .modal-overlay {
                visibility: hidden;
                margin: 0;
                padding: 0;
            }
        ...
        #printable-receipt h2 { font-size: 16pt; text-align: center; margin: 0; padding: 0; }
        #printable-receipt p { text-align: center; margin: 2px 0; font-size: 10pt; }
        #printable-receipt table { width: 100%; border-collapse: collapse; margin-top: 10px; }
        #printable-receipt th, #printable-receipt td { padding: 4px 2px; word-break: break-all; }
        #printable-receipt th { border-bottom: 2px solid #000; font-weight: bold; text-align: left; }
        #printable-receipt .text-right { text-align: right; }
        #printable-receipt .total-row td { border-top: 2px solid #000; font-weight: bold; padding-top: 5px; }
        #printable-receipt .footer-thanks { text-align: center; margin-top: 10px; font-size: 10pt; }
    </style>
</head>
<body>
    <div class="container">
        <header>
            <div class="header-content">
                <div class="logo"><i class="fas fa-cash-register"></i><div class="logo-text"><h1>POS System</h1><p>Remkon Store</p></div></div>
            </div>
        </header>
        ...
        <div class="layout">
            <div class="products-section">
                ...
                <div class="products-grid" id="products-grid">
                    <?php if ($products_result && $products_result->num_rows > 0): ?>
                        <?php while ($product = $products_result->fetch_assoc()): ?>
                            <div class="product-card <?php if ($product['stock_quantity'] <= 0) echo 'out-of-stock'; ?>"
                                 ...
                                 data-pieces-per-carton="<?= $product['pieces_per_carton'] ?? 1 ?>">
                ...
                <div class="load-more-container" id="load-more-container">
                    <button id="load-more-btn" class="btn-load-more">Load Out-of-Stock Products</button>
                </div>
            </div>

            <div class="cart-container">
                <div class="cart-header"><h2><i class="fas fa-shopping-cart"></i> Current Sale</h2></div>
                <div class="cart-body" id="cart-items"><div class="empty-cart"><i class="fas fa-shopping-cart"></i><h3>Your cart is empty</h3><p>Click on a product to start a sale</p></div></div>
                    ...
                    <form id="sale-form" method="POST">
                        <div class="form-group">
                            ...
                            <datalist id="customers-list">
                                <?php while($customer = $customers_result->fetch_assoc()): ?>
                                    <option value="<?= htmlspecialchars($customer['name']) ?> (<?= htmlspecialchars($customer['phone']) ?>)" data-id="<?= $customer['id'] ?>"></option>
                                <?php endwhile; ?>
                            </datalist>
                        ...
                        <div class="form-group">
                            ...
                            <select name="payment_method" id="payment_method" class="form-control">
                                <option value="cash">Cash</option><option value="credit_card">Credit Card</option><option value="debit_card">Debit Card</option>
                            </select>
                        </div>
                        <div class="btn-group">
    ...
    <div id="pos-modal" class="modal-overlay">
        <div class="modal-content">
            <div class="modal-header"><h2 id="modal-product-name">Product Name</h2><button type="button" class="modal-close">&times;</button></div>
            <div class="modal-body">
                ...
                <div class="quantity-grid">
                    <div class="form-group"><label for="modal-carton-quantity">Quantity (Cartons)</label><input type="number" id="modal-carton-quantity" class="form-control" value="0" min="0"></div>
                    <div class="form-group"><label for="modal-piece-quantity">Quantity (Pieces)</label><input type="number" id="modal-piece-quantity" class="form-control" value="1" min="0"></div>
                </div>
                <div class="form-group"><label for="modal-price">Selling Price (per piece)</label><input type="number" id="modal-price" class="form-control" step="0.01" min="0" placeholder="Enter manual price"></div>
            </div>
            <div class="modal-footer"><button type="button" class="btn btn-secondary modal-close">Cancel</button><button type="button" id="add-to-cart-btn" class="btn btn-primary">Add to Cart</button></div>
        </div>
    </div>
    
    <div id="receipt-modal" class="modal-overlay">
        <div id="receipt-modal-content" class="modal-content">
            <div class="modal-header"><h2>Receipt</h2><button type="button" class="modal-close">&times;</button></div>
            <div id="receipt-body" class="modal-body"></div>
            <div class="modal-footer"><button type="button" class="btn btn-secondary modal-close">New Sale</button><button type="button" id="print-receipt-btn" class="btn btn-primary"><i class="fas fa-print"></i> Print</button></div>
        </div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const modalCartonQtyInput = document.getElementById('modal-carton-quantity');
        const modalPieceQtyInput = document.getElementById('modal-piece-quantity');
        ...
        const addToCart = async () => {
            const cartonQty = parseInt(modalCartonQtyInput.value, 10) || 0;
            const pieceQty = parseInt(modalPieceQtyInput.value, 10) || 0;
            const price = parseFloat(modalPriceInput.value);
            const piecesPerCarton = currentProduct.piecesPerCarton;
            const totalQuantity = (cartonQty * piecesPerCarton) + pieceQty;
            if (totalQuantity <= 0) { alert('Please enter a valid quantity.'); return; }
        ...
        const processSale = async () => {
            ...
            const result = await apiRequest('../api/sale.php', 'POST', { customer_id: customerId, payment_method: paymentMethod });
            if (result && result.success) showReceipt(result.receipt);
        };

        const showReceipt = (receiptData) => {
            let itemsHtml = `<h2>${receiptData.store_name || 'Remkon Store'}</h2><p>Receipt ID: ${receiptData.id}</p><p>Date: ${new Date(receiptData.date).toLocaleString()}</p><table id="receipt-items-table"><thead><tr><th>Item</th><th>Qty</th><th>Price</th><th>Total</th></tr></thead><tbody>`;
            receiptData.items.forEach(item => {
                itemsHtml += `<tr><td>${item.name}</td><td>${item.quantity}</td><td>${item.price.toFixed(2)}</td><td>${(item.quantity * item.price).toFixed(2)}</td></tr>`;
            });
            ...
            let sn = 1;
            receiptData.items.forEach((item) => {
                printableHtml += `<tr><td>${sn++}</td><td>${item.name}</td><td>${item.quantity}</td><td class="text-right">${item.price.toFixed(2)}</td><td class="text-right">${(item.quantity * item.price).toFixed(2)}</td></tr>`;
            });
            printableHtml += `<tr class="total-row"><td colspan="4">TOTAL</td><td class="text-right">₦${receiptData.total.toFixed(2)}</td></tr></tbody></table><p>----------------------------------------</p><p class="footer-thanks">Thank you for your patronage!</p>`;
        ...
        const openPosModal = (productCard) => {
            ...
            currentProduct = { id: productCard.dataset.productId, name: productCard.dataset.productName, defaultPrice: parseFloat(productCard.dataset.productPrice), costPrice: parseFloat(productCard.dataset.costPrice), stock: parseInt(productCard.dataset.stockQuantity, 10), piecesPerCarton: parseInt(productCard.dataset.piecesPerCarton, 10) || 1 };
            modalProductNameEl.textContent = currentProduct.name;
            modalCostPriceEl.textContent = `₦${currentProduct.costPrice.toFixed(2)}`;
            modalCartonQtyInput.value = 0;
            modalPieceQtyInput.value = 1;
            modalPriceInput.value = currentProduct.defaultPrice.toFixed(2);
            modalCartonQtyInput.disabled = currentProduct.piecesPerCarton <= 1;
            posModal.style.display = 'flex';
            modalPieceQtyInput.focus();
        };

        const closePosModal = () => { posModal.style.display = 'none'; };
        ...
        const filterProducts = () => {
            ...
            allProductCards.forEach(card => {
                ...
                const stockStatus = card.classList.contains('out-of-stock') ? 'out' : 'in';

                let isVisible = true;
                if (loadMoreContainer.style.display !== 'none' && stockStatus === 'out') {
                    isVisible = false;
                }
        ...
        allProductCards.forEach(card => card.addEventListener('click', () => openPosModal(card)));
